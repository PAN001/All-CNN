...
for layerCnt in layers:
    ...
    sess.run(locals()[layerName], feed_dict={X: batch_LSUV, Y: training_labels_shuffled})  ## run one forward pass
    ...
    mean, var = tf.nn.moments(locals()[weightsName], axes=[0, 1, 2, 3])  # get the variance
    print("cur var is: ", var.eval())

    iter = 0
    target_var = 1.0 # the targeted variance

    while (abs(target_var - var.eval()) > margin):
